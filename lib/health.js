...
class HealthMonitor {
    ...
    _runHealthChecks() {
        ...
        const ping = Number(this.client.ws?.ping) || 0;
        const pingHealthy = Number.isFinite(ping) && ping >= 0 && ping < 2000;
        const connectionPassed = this.client.isReady() && pingHealthy;

        checks.push({
            name: 'Discord Connection',
            passed: connectionPassed,
            message: this.client.isReady() ? (ping > 0 ? 'Connected' : 'Connecting') : 'Disconnected',
            value: `${ping}ms`
        });

        // Memory check
        const rssPercent = systemTotalMemory > 0
            ? (processMemory.rss / systemTotalMemory) * 100
            : 0;
        const heapPercent = processMemory.heapTotal > 0
            ? (processMemory.heapUsed / processMemory.heapTotal) * 100
            : 0;
        const memoryHealthy = rssPercent < 85 && heapPercent < 98;
        checks.push({
            name: 'Memory Usage',
            passed: memoryHealthy,
            message: memoryHealthy ? 'Normal' : 'High',
            value: `RSS ${rssPercent.toFixed(2)}% • Heap ${heapPercent.toFixed(2)}%`
        });
